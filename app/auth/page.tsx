...
import { Eye, EyeOff, KeyRound, Loader2, Phone, RefreshCw, ShieldCheck, X } from 'lucide-react';
import { getCaptcha, login, loginWithPassword, sendSms } from '@/lib/api';
...
export default function LoginPage() {
    ...
    const handleLogin = async () => {
        setError('');
        if (!smsCode || smsCode.length !== 6) { setError('请输入6位短信验证码'); return; }
        setLoading(true);
        try {
    ...
    const handlePasswordLogin = async () => {
        ...
        setLoading(true);
        try {
            const tokens = await loginWithPassword(phone, password, captchaId, captchaText);
            await authLogin(tokens.access_token, tokens.refresh_token);
            router.replace('/');
        } catch (e: unknown) {
            ...
            >
                ...
                <div className="flex gap-1 p-1 bg-slate-100 rounded-xl mb-6">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'sms' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <Phone className="w-3.5 h-3.5" /> 短信验证码
                    </button>
                    <button
                        onClick={() => switchMode('password')}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'password' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <KeyRound className="w-3.5 h-3.5" /> 账号密码
                    </button>
                </div>
                ...
                {mode === 'sms' && (
                    step === 'phone' ? (
                        <div className="space-y-4">
                            ...
                            <CaptchaField captchaImg={captchaImg} captchaText={captchaText}
                                onChange={setCaptchaText} onRefresh={fetchCaptcha} />
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" maxLength={6} value={smsCode}
                            ...
                            <div className="flex gap-3">
                                ...
                                {countdown > 0 ? (
                                    <span className="px-4 py-3 text-sm text-slate-400 border border-slate-200 rounded-xl">{countdown}s</span>
                                ) : (
                                    <button onClick={() => { setStep('phone'); void fetchCaptcha(); }}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type={showPassword ? 'text' : 'password'} value={password}
                                    ...
                                    placeholder="请输入密码"
                                    className="w-full pl-10 pr-10 py-2.5 text-sm text-slate-800 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 bg-slate-50" />
                ...
                {showTerms && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                            exit={{ opacity: 0, scale: 0.95, y: 16 }} onClick={(e) => e.stopPropagation()}
...
function CaptchaField({ captchaImg, captchaText, onChange, onRefresh }: {
    captchaImg: string;
    captchaText: string;
    ...
    return (
        <div>
            ...
            <div className="flex gap-3">
                <input type="text" maxLength={4} value={captchaText}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={onRefresh}
                        className="p-2 rounded-lg hover:bg-slate-100 transition text-slate-400" title="刷新验证码">
